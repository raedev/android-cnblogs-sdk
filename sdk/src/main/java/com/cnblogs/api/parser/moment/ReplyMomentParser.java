 ...
 */
public class ReplyMomentParser implements IHtmlParser<List<MomentCommentBean>> {

    @Override
    public List<MomentCommentBean> parse(Document document) throws IOException {
        List<MomentCommentBean> result = new ArrayList<>();
        Elements elements = document.select(".comment-list");
        for (Element element : elements) {
            ...
            m.content = element.select(".ing_body").text().replace("：", "");
        }
